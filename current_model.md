# Current Hetero Fraud Model (Notebook-Style Cells, Leak-Safe Temporal Context)
...
def build_relation_mappings(df, relation_cols, min_freq_by_col, default_min_freq=100):
    ...
    for col in relation_cols:
        ...
        mappings[col] = {"mapping": mapping, "frequent_values": frequent, "num_nodes": len(mapping)}
    return mappings
...
def encode_relation_values(series, mapping_info):
    ...
    codes = s.map(mapping_info["mapping"]).fillna(mapping_info["mapping"]["__RARE__"]).astype(np.int64)
...
def prepare_base_dataframe(
    ...
    relation_mappings = build_relation_mappings(
        df, relation_cols, min_freq_by_col=min_freq_by_col, default_min_freq=default_min_freq
    )
...
```python
def pick_edge_type(data_obj, preferred_rel="has_card1"):
...
for name, graph in [("Train context", train_data), ("Val context", val_data), ("Test context", test_data)]:
    et = pick_edge_type(graph, preferred_rel="has_card1")
...
## 8) Define Model
```python
class FraudHeteroSAGE(nn.Module):
    def __init__(self, data, hidden_channels=32, num_layers=2, dropout=0.2, aggr="sum"):
        super().__init__()
        ...
        for _ in range(num_layers):
            conv_dict = {et: SAGEConv((hidden_channels, hidden_channels), hidden_channels) for et in self.edge_types}
...
def plot_training_curves(history, save_dir=None):
    ...
    plt.title("Training Loss Curve"); plt.xlabel("Epoch"); plt.ylabel("Loss"); plt.grid(alpha=0.2); plt.legend()
    if save_dir is not None: plt.savefig(os.path.join(save_dir, "loss_curve.png"), dpi=140, bbox_inches="tight")
    plt.show()
    ...
    plt.title("Validation Metrics by Epoch"); plt.xlabel("Epoch"); plt.ylabel("Score")
    plt.grid(alpha=0.2); plt.legend()
    if save_dir is not None: plt.savefig(os.path.join(save_dir, "val_metrics_curve.png"), dpi=140, bbox_inches="tight")
    plt.show()
```
...
for epoch in range(1, EPOCHS + 1):
    ...
    if need_raw:
        plot_roc_curve(
            ...
            title=f"Validation ROC Curve - Epoch {epoch}",
        ...
        print(f"Validation ranking metrics @ epoch {epoch}:")
